...
In this proof-of-concept, we successfully combined **medical imaging, machine learning, and NLP** into a unified pipeline for neuroendocrine tumor analysis. By using the MSKCC DOTATATE PET/CT pretrained model, we achieved robust automated segmentation without needing proprietary data. The radiomics and classification step, while demonstrated on a basic level here, opens the door for predicting clinical outcomes (e.g., patient survival or therapy response) if integrated with real labels ([Frontiers | Radiomics-Based Texture Analysis of 68Ga-DOTATATE Positron Emission Tomography and Computed Tomography Images as a Prognostic Biomarker in Adults With Neuroendocrine Cancers Treated With 177Lu-DOTATATE](https://www.frontiersin.org/journals/oncology/articles/10.3389/fonc.2021.686235/full#:~:text=Conclusion%3A%20These%20preliminary%20data%20generate,to%20the%20assessment%20of%20patients)). The NLP report summarizer highlights how text data can be incorporated, potentially assisting radiologists by producing quick summaries or cross-validating the image findings with the written report.

This modular design allows for many **extensions**. For instance, one could incorporate *multi-modal learning*: using the image features and text together (maybe via a multimodal transformer) to see if the report’s text can help refine the image analysis or vice versa. Another idea is to integrate an **interactive viewer** where a user can click on a summary sentence and see the corresponding lesions highlighted in the image (image-text alignment). Moreover, as new public datasets emerge (for example, if MSKCC or others release sample DOTATATE scans or if synthetic data generation improves), the pipeline can be adapted to those with minimal effort.

In summary, this project demonstrates a full-stack approach — from DICOM to diagnosis summary — for AI-driven analysis of neuroendocrine tumors. We provide the code, documentation, and examples needed for others to reproduce and build upon this work. We hope it serves as a solid foundation for future research and development at the intersection of medical imaging and natural language processing.
